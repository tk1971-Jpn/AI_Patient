...
PROMPT_VERSION = "case_ac_chole_hybrid_v2"
TEMPERATURE = 0.2
USE_LLM = True
...
def update_check_items(user_text: str, check_items: Dict[str, bool]) -> Dict[str, bool]:
    ...
    if contains_any(t, ["どんな痛み", "性状", "ズキズキ", "締め付け", "刺す", "重い感じ", "痛みの感じ"]):
...
def generate_patient_response_rule(user_text: str) -> str:
    ...
    if contains_any(t, ["どこが痛", "場所", "部位", "どのあたり", "痛む場所"]):
...
def generate_patient_response_llm(user_text: str, messages: List[Dict], case_data: Dict) -> str:
    ...
    history_block = "\n".join(conversation_text) if conversation_text else "（まだ会話なし）"
...
- 質問に対して患者として答え、指示文や解説文を書かないでください。
""".strip()
